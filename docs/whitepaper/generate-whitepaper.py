...
Above the storage substrate, a Services layer provides a trait-based framework
for stateless servers that run on ZODES, use Programs for persistent state via a key-value
...
<tr><td><strong>ServiceDescriptor</strong></td><td>A CBOR map declaring a service&rsquo;s name, version, required programs, and owned programs.</td></tr>
<tr><td><strong>ProgramStore</strong></td><td>A key-value abstraction over sector append-logs provided to services. Keys map to SectorIds via <code>SHA-256(key)</code>; values are sector log entries.</td></tr>
</table>
...
<table>
<tr><th>Field</th><th>Type</th><th>Description</th></tr>
<tr><td><code>name</code></td><td>text string</td><td>Human-readable service name</td></tr>
<tr><td><code>version</code></td><td>text string</td><td>Semantic version</td></tr>
<tr><td><code>required_programs</code></td><td>list of ProgramId</td><td>Programs this service reads/writes (must already exist)</td></tr>
...
<tr><td><code>descriptor()</code></td><td>Returns the service&rsquo;s <code>ServiceDescriptor</code>; declares identity and program dependencies.</td></tr>
<tr><td><code>routes(ctx)</code></td><td>Returns an HTTP router mounted at <code>/services/{service_id_hex}/</code>.</td></tr>
<tr><td><code>on_start(ctx)</code></td><td>Called after ZODE boot; may spawn background tasks using a cancellation token for graceful shutdown.</td></tr>
...
<ul>
<li><strong>ProgramStore</strong>&mdash;a key-value abstraction over sector append-logs. Keys map to <code>SectorId</code> via <code>SHA-256(key)</code>. <code>put</code> appends a new entry (latest entry = current value). <code>get</code> reads the last entry. <code>list</code> reads the full log history.</li>
<li><strong>Ephemeral tokens</strong>&mdash;HMAC-SHA256 signed, time-limited tokens for stateless short-lived flows (auth challenges, OAuth nonces). The client holds the token and presents it back; no server-side storage is required.</li>
<li><strong>Event broadcast</strong>&mdash;services emit <code>Started</code>, <code>Stopped</code>, and <code>RequestHandled</code> events.</li>
<li><strong>Shutdown token</strong>&mdash;cooperative cancellation for background tasks.</li>
</ul>

<h4>ProgramStore Operations</h4>

<table>
<tr><th>Operation</th><th>Behavior</th></tr>
<tr><td><code>get(key)</code></td><td>Derive <code>SectorId = SHA-256(key)</code>, read last log entry.</td></tr>
<tr><td><code>put(key, value)</code></td><td>Derive <code>SectorId</code>, append value to sector log.</td></tr>
<tr><td><code>list(key)</code></td><td>Read all entries for the derived sector (full history).</td></tr>
<tr><td><code>list_from(key, from)</code></td><td>Read entries starting from a given index.</td></tr>
...
<ul>
<li><strong>Register</strong>&mdash;compute <code>ServiceId</code>, reject duplicates, store the service.</li>
<li><strong>Start all</strong>&mdash;create a <code>ServiceContext</code> per service, call <code>on_start</code>, emit <code>Started</code> event.</li>
<li><strong>Stop all</strong>&mdash;cancel background tasks, call <code>on_stop</code>, emit <code>Stopped</code> event.</li>
<li><strong>Merged router</strong>&mdash;build a combined HTTP router with each service nested at <code>/services/{service_id_hex}/</code>.</li>
...
<p><strong>Host-provided imports:</strong></p>

<table>
<tr><th>Interface</th><th>Functions</th><th>Description</th></tr>
<tr><td><code>store</code></td><td><code>get</code>, <code>put</code>, <code>list-entries</code>, <code>list-from</code>, <code>entry-count</code></td><td>Key-value access to Grid Programs</td></tr>
<tr><td><code>ephemeral</code></td><td><code>create-token</code>, <code>verify-token</code></td><td>Signed time-limited tokens</td></tr>
</table>

<p><strong>Guest-exported interface:</strong></p>

<table>
<tr><th>Function</th><th>Description</th></tr>
<tr><td><code>get-descriptor()</code></td><td>Returns the service descriptor</td></tr>
<tr><td><code>handle-request(req)</code></td><td>Processes an HTTP request and returns a response</td></tr>
<tr><td><code>on-start()</code></td><td>Lifecycle hook at service startup</td></tr>
<tr><td><code>on-stop()</code></td><td>Lifecycle hook at service shutdown</td></tr>
</table>
...
<table>
<tr><th>Service</th><th>Required Programs</th><th>Endpoints</th></tr>
<tr><td><strong>Identity</strong> (v1.0.0)</td><td>ZID v1</td><td><code>POST /resolve</code>, <code>GET /health</code></td></tr>
<tr><td><strong>Interlink</strong> (v1.0.0)</td><td>Interlink v2</td><td><code>GET /messages</code>, <code>GET /health</code></td></tr>
</table>
...
<tr><td>Nature</td><td>Passive descriptor (schema + proof config)</td><td>Active process (server, compute)</td></tr>
<tr><td>State</td><td>Defines storage format</td><td>Stateless; reads/writes via Programs</td></tr>
<tr><td>Identity</td><td><code>ProgramId = SHA-256(descriptor)</code></td><td><code>ServiceId = SHA-256(descriptor)</code></td></tr>
<tr><td>Runs on</td><td>Nothing (specification only)</td><td>ZODE nodes</td></tr>
...
<li>Build <code>ServiceRegistry</code>; register default services (Identity, Interlink).</li>
<li>Compute effective topic set: default programs &cup; configured programs &cup; service-required programs.</li>
<li>Subscribe to each GossipSub topic.</li>
<li>Start all registered services (<code>on_start</code>); build merged HTTP router.</li>
<li>Start RPC server with sector endpoint and service routes merged.</li>
...
<tr><td>Service requests</td><td>Yes &mdash; ZODE routes HTTP to services</td></tr>
</table>
...
<li><strong>WASM service isolation.</strong> Sandboxed WASM services are subject to fuel metering, memory limits, and wall-clock timeouts. A misbehaving module cannot affect the ZODE or other services, but resource exhaustion within limits is possible.</li>
