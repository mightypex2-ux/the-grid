...
pub use grid_crypto::{derive_sector_id, CryptoError};
...
pub fn sector_encrypt(
    ...
    let ciphertext =
        grid_crypto::encrypt_sector(&padded, sector_key, &aad).map_err(SdkError::Crypto)?;
    Ok(ciphertext)
}
...
pub fn sector_decrypt(
    ...
    let aad = build_sector_aad(program_id, sector_id);
    let padded =
        grid_crypto::decrypt_sector(ciphertext, sector_key, &aad).map_err(SdkError::Crypto)?;
    ...
    F: FnOnce(&str) -> Option<zero_neural::IdentityVerifyingKey>,
{
    ...
    let msg: grid_programs_interlink::ZMessage =
        grid_core::decode_canonical(&plaintext).map_err(SdkError::Core)?;
